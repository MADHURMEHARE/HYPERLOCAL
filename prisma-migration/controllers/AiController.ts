...
import { PREDICTIVE_HOTSPOTS } from '../../server/db';
...
Be analytical. Respond ONLY with the valid JSON. No Markdown formatting, no code block wrapping, just raw, valid JSON.`;
  ...
  static async checkDuplicate(req: Request, res: Response) {
    ...
    try {
      ...
      const nearbyIssues = allIssues.filter(i => {
      ...
      return res.status(500).json({ error: error.message || 'Duplicate check failed.' });
    }
  }
...
Do not use Markdown wrapping, respond with ONLY raw JSON.`;
  ...
  static async chat(req: Request, res: Response) {
    ...
    try {
      ...
      const allIssues = await IssueRepository.getAll();
      const activeIssuesSummary = allIssues.slice(0, 6).map(i => 
