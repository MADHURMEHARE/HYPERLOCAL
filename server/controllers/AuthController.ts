...
import { JWT_SECRET, DEFAULT_PASSWORD_HASH } from '../db';
...
export class AuthController {
  static register(req: Request, res: Response) {
    ...
    const newUser = UserRepository.create(name, email, passwordHash, role as UserRole, ward);
  ...
  static login(req: Request, res: Response) {
    ...
    if (!password || isDemo) {
      ...
      const newUser = UserRepository.create(name, email, DEFAULT_PASSWORD_HASH, (role as UserRole) || 'citizen', 'Ward 3 - Mission');
      const token = jwt.sign({ id: newUser.id, email: newUser.email, role: newUser.role }, JWT_SECRET, { expiresIn: '24h' });
